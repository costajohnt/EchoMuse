...
- 2026-08-21 (**the Amazon question, answered by declining the best argument against it**): PR #168 closed. The native-AFE branch routed device audio through Amazon's own ASP pipeline instead of our tinyalsa writes, and it was the strongest possible case for doing so: opt-in per device, default off, the old path untouched, 4600 lines of real reverse engineering with a spec, a phase plan and kill criteria, and — by its author's ear and mine — better sounding. It was declined anyway, because the thing it did well was make Amazon's HAL the path the audio takes, and the project's direction (stated 2026-08-18) is to run on hardware that is not an Echo, without Amazon's software underneath.
  ...
  **A triage of everything open, and five things closed off it.** 61 issues and 9 PRs read in one pass. Merged: #192 (EA changelog context), #264 (the denoiser floor), #209 (#191's first piece — every device gets all four stock classifiers, plus reconcile-on-connect). Closed: #82, which was **already fixed** and nobody had said so — the wizard classifies WPA3 and refuses it with the reason since the security-flags work; and #88, whose reporter had solved their own case and written up why. #168 declined as above. Two issues filed from what the triage surfaced rather than from new work: #268 and #269.

  **The reply-writing itself took five drafts, and the corrections generalise.** Wil cut, in order: the contrastive "we're not doing X, it's about Y" construction (*"a classic AI meme now"*), a superlative opener (*"slimy and sycophantic"*), an inverted sentence (*"who are you? Yoda?"*), an apology attached to a request (*"you're a strong, independent AI, you don't need to simper"*), and "I won't pretend it's close" — performative honesty, which narrates candour instead of being candid. What survived: specific appreciation without superlatives, a real apology for cost the contributor actually bore, and somewhere concrete to land at the close. He also named the attribution problem underneath it: everything posts from his account, so a drafted first-person claim invents his experience. Rule now: quote a measurement he actually posted, or phrase it neutrally.

  **`f1r30s.zip` turned out to be four things, three of them undocumented here.** Both docs called it an "ADB enablement patch". Reading the copy Wil supplied (sha256 `f1e522fb…`) shows the whole payload is one `/sbin/sh` script: it strips `verify` from the ramdisk fstab (dm-verity off), patches `default.prop` (`ro.secure=0`, `ro.debuggable=1`, `persist.sys.usb.config=mtp,adb`), **repacks and re-flashes the boot partition**, patches `/system/etc/init.fosflags.sh` to force ADB on and set `amazon.fos_flags.noadbauth 1`, and blackholes four Amazon update domains in `/system/etc/hosts`. Three consequences. The re-flash means it must run **before** our `patch_boot`, or it repacks away the byte-64 cmdline edit magiskboot cannot make. `noadbauth` living in `/system` is what makes wiping `/data` safe on a screenless device — `adb_keys` is in `/data`, and there is no way to accept an authorisation prompt on a Dot. And the hosts patch is why a provisioned device never updates itself out from under EchoMuse, which nothing recorded.

  **Wil's step 0 — wipe data, wipe cache, install f1r30s — is field-proven and needs no firmware sideload.** `/system` is populated by the sideload during the unlock; a data/cache wipe does not touch it. Upstream's own flow uses `twrp wipe data` and `twrp wipe cache` as standalone commands, and the thread's note that you must flash f1r30s *after* a stock firmware image explains why: a stock flash restores dm-verity. Filed as #269 Part 1. The case for it was already in this journal — v2.5.1 records two bugs caused by `/data` surviving a re-provision (stale `server_a`/`server_b` binaries; a stale `magisk.img` that made every `su` take ~60s), each patched with a targeted `rm -f`. A wipe removes the class. It also makes `f1r30s.zip` something the wizard supplies rather than something the user must remember to push, which is the whole of #88.

  **The wizard verifies where we have been hurt and trusts everywhere else** (#268). `patch_boot` reads the cmdline back off the partition rather than trusting `dd`, and `install_magisk` hashes the zip before flashing — the two places somebody got burnt. Everywhere else takes an exit code as proof: `unzipOut` is logged and never inspected, and `cp && chmod` proves `cp` returned 0 rather than that the source had bytes in it. Also found: `Cancel step` renders on `{running && …}` with no exclusion by step, so it is on screen during the one `dd` that must not be interrupted — and it cannot stop it anyway, because `abandoned()` is only consulted after the step body resolves. What cancelling there actually does is discard the readback verification, leaving the operator a Retry button and no "do not reboot" warning.

  **A mechanism for #79, and a correction to my own claim.** `f1r30s` sets `persist.sys.usb.config` in two places — `mtp,adb` in the ramdisk `default.prop`, and an append in the fosflags injection. @kylegordon's Dot reported exactly `adb`, which is what the fosflags path produces when the prior value was `none`: that half ran and the `default.prop` half did not. I then wrote that a different FireOS build was "still the other live explanation" as though it were established — it never was. The fingerprint they sent on the 3rd is TWRP's own build, because the question was asked while they were sat in recovery, so the Dot's FireOS build has never been captured. Corrected on the issue with the six builds the thread lists.

  **#167 (timers) reviewed, and three things I expected to be wrong were not.** The `_run_post_turn_playback` change is a faithful re-indent inside `try/finally` with every comment intact; the LED is gated on `led_anim_capable`; and the amber `(255,170,0)` at 500ms is genuinely distinct from link-down orange `(255,40,0)` at 2000ms — checked rather than assumed, and the assumption would have been wrong. The one real finding: `is_dismissal` matches generously *and* `_dismissed_alarm` then suppresses HA's reply for the whole turn, so "turn off the kitchen light" spoken over a ringing alarm turns the light off and swallows the confirmation. Their generosity argument is right for stopping the ring and does not carry to suppressing the reply. Their own non-dismissal test uses "turn **on** the kitchen light".

  **`docs/audio-states.md` written, because a fourth thing now wants the speaker.** Voice, music, announcements and — with #167 — a timer alarm, each added on its own and correct in isolation. Same shape as `led-ring-states.md`: principles, an owner priority ladder, transition tables marked `[today]`/`[proposed]`, open questions, invariants. Two things surfaced only in the writing. `0x04` and `0x05` mean different things by direction (music PCM / music EOS down, VAD end / no-speech timeout up) and are disambiguated solely by which way the frame travels. And **#262's rule is older than the thing that justifies it**: "a voice turn owns the speaker unconditionally" was written when music shared the voice plane, so music starting mid-turn would have talked over the response — music has had its own plane since ducking shipped, and the reason is now much weaker. Recorded as an open question rather than changed.

- 2026-08-22/23 (**nine contributions overnight, and the one that had already been fixed**): @chr-braun sent nine PRs against filed issues, in this repo's own house style, with tests. Eight merged. #277 was a duplicate of `ab31aef` — the firmware build fix done the previous morning on the `sendspin-design` branch — and it is the reason the batch is worth recording: the duplicate was not wasted, because **it was right about a detail we were wrong about.** `ab31aef` pinned `golang.org/x/sys` to v0.40.0 and asserted in its own commit message that v0.40.0 was the newest release still declaring `go 1.24.0`. It is not. Checked against the module proxy rather than asserted a second time: v0.40.0 and v0.41.0 both declare 1.24.0, and v0.42.0 is where the jump happens. The PR pinned v0.41.0. So the close is "already fixed", and the one line worth taking came from the duplicate. Our CI job survives instead of theirs because it additionally checks the build produced an ARM binary — a build that silently emitted a host binary would pass the build step and ship something the hardware cannot run.

  **The review found one real defect, and it was in the safest-looking change.** #282 records a turn cancelled mid-response as `barged` instead of the finally-default `ok`, which is correct and well argued — an outcome that flatters the system is the worst kind to get wrong, and this one hid #250 for two days. But it dropped the `if trace:` guard that every sibling assignment in that function carries, on a parameter whose default is `None` and whose contract is written down four hundred lines up: *"may be None — all trace.x calls guard against this"*. Not currently reachable, because `trigger_voice_turn` is the only caller and always constructs a trace. Restored anyway, in a separate commit: the exposure sits on exactly the path the change is about, and an invariant that holds only by the accident of there being one call site is not holding.

  **The biggest diff was the least risky.** #280 is +498/−389 in `em_controller.py` and almost all of it is re-indentation: the control-plane dispatch chain wrapped in a per-message `try/except`, so one bad message stops costing the device its voice satellite, its BLE proxy and its data plane. `CancelledError` still propagates and `register` stays fatal, both deliberately. The data plane gets the same guard rate-limited to one traceback per 5s, because it runs at frame rate. Worth noting what *cannot* verify this: the controller test suite deliberately does not import `em_controller`, so an 834-line re-indent in that file has no test that would catch a broken control flow. It was smoke-tested by booting the container instead.

  **Two fixes verified by running them rather than by reading them.** #279 — `update_check_interval=0` meant "poll as fast as possible" rather than "stop", because `db.get_config` returns a string and `"0"` is truthy, so the guard fell through to `asyncio.sleep(0)`. The worst available outcome for exactly the person who set it to stop the controller talking to GitHub. Confirmed fixed on a booted container: one poll on the default interval, zero across two minutes with the value set to `0`. And the merge itself needed care — #278 and #283 both insert constants immediately after `SOURCE_STALL_MS`, and a naive both-sides conflict resolution spliced two test function bodies together into something that still looked plausible and did not parse. Caught on a syntax check, redone by appending the added tests cleanly.

  **A mistake worth writing down because it is the same class as the one being fixed.** Correcting the x/sys pin to v0.41.0, I edited the version string in `go.sum` and left the previous release's `h1:` hash on the line. That produces a file that looks correct to a reader and fails verification at build time. The right values came from `sum.golang.org`. It is the identical failure to the one #277 existed to fix — a pin that is wrong in a way no test in the tree looks at — made while fixing it.


- 2026-08-23, afternoon (**a dependency bump that broke a feature and said nothing**): deploying a locally built firmware failed with "an internal error occurred". Root cause: aiohttp's `client_max_size` defaults to 1 MB, `create_app` never set it, and the firmware is ~10.7 MB — so `/api/releases/upload` rejected the body before the handler ran. Both callers post there, the dashboard's Local Build panel and `controller/tools/ota.py`.

  **It is a regression, and establishing that changed what to distrust.** The first commit message said the endpoint "could never accept a real binary", which was wrong and would have been recorded as fact. Wil asked whether an aiohttp bump caused it; measured across the two pinned versions with a 3 MB multipart POST at a default Application — 3.13.5 returns **200** (streaming multipart bypassed the limit), 3.14.3 returns **413**. 3.13.5 held from May until 2026-08-18, when #129's routine half moved to 3.14.3. Local deploys worked for the life of the project until five days ago.

  **Three reasons nothing caught it, and the third is the general one.** The handler carries its own 50 MB check, so the code reads as though large uploads were handled — that check sits after the transport limit and never ran. The ordinary release path never touches the endpoint, since `em_firmware` fetches controller-side and pushes from there, so only a developer deploying a local build meets it. And **a dependency bump changed BEHAVIOUR rather than an interface**, which no import, no type and no green check can see. That is the same class as #224 (numpy/onnxruntime bumps needing a wake word comparison behind them), and it is the argument for #293's screenshot baselines: the checks worth having are the ones that do not encode anyone's intent.

  The transport limit is now set deliberately **above** the application limit (`UPLOAD_MAX_BYTES + 8 MB`), so the handler's 413 — which names the real ceiling and the size sent — is an error a user can reach. Equal limits make the useful message unreachable by construction. The handler also re-raises `web.HTTPException` before its general clause; that blanket `except Exception` is why a size limit presented as a 500 with no number in it.

  **The morning, for the record**: eight of @chr-braun's nine overnight PRs merged, #277 closed as a duplicate that was nonetheless right about the `x/sys` pin our own commit got wrong. CI gained a job that imports every controller module and boots the controller, because the suite deliberately cannot import `em_controller` or `em_esphome` and #280 had just re-indented 834 lines inside one of them. A second update-check hole closed on the no-cache branch #279 left open — the fresh-install path, which is exactly where someone sets the interval to 0 on day one. All 58 open issues labelled by what blocks them, which turned out to be only 10.

- 2026-08-23, late afternoon (**the first thing all day validated by ear, and the bug it turned up on the way**): the local wake ring (#284) is confirmed working on hardware — the ring lights as the wake word is said, rather than after the +522ms round trip it used to take. Confirmed on the bedroom device, at the far end of the house from any access point, which is the hardest place in the building to have got it right.

  **Getting there took three bugs that all presented as something else.** The firmware upload failed with "an internal error occurred" (aiohttp's 1 MB body limit against a 10.7 MB binary — a regression from the 3.13.5 → 3.14.3 bump five days earlier, measured across both versions rather than assumed). Then the deploy reported `⚠ Device reconnected on v2.12.0-63-g99628d3 — auto-rolled back` while having succeeded perfectly: `_extract_binary_version` matches the OLD date-based version scheme (`20260614-1152-dev`) and `compile.sh` has since moved to `git describe`, so every clean-tree local build is labelled `local-<timestamp>`, never matches what the device reports, and reads as a rollback. A message that says the opposite of what happened is worse than a failure, because the natural response is to stop trusting the feature.

  **And #251 turned out to fix the case it was not written for.** A real barge cut "Sing a song." off mid-song and the turn persisted `outcome=ok`. There are two barge paths: during THINKING goes through `cancel_voice_turn` and sets `_turn_cancelled`; during PLAYBACK goes through `abort_ha_run`, which deliberately does not, because that flag gates `on_thinking`, `on_stt_end` and the intent-ended check and marking a delivered response cancelled would change behaviour rather than only the record. That reasoning is right and was kept; the fix now records a separate `_turn_barged` carrying no control-flow meaning, and the outcome reads both. The test covers both paths, verified by reintroducing each half — including the per-turn reset, since a sticky flag is the same wrong answer in the other direction.

  **The bedroom is now the best test rig this project has.** Idle RTT excursions to **4.8s** against #139's worst of 1.4s, two `keepalive ping timeout` disconnects, the data plane cycling five times, `sub_drops` to 64. The controller was healthy throughout — one 273ms loop stall, no tracebacks — and both voice turns completed. Nothing was broken except the link, and everything downstream of it.

  Four things came out of that, none of them regressions and all filed for 3.0.0: the wake-stream watchdog reads a dropped link as a broken mic and escalates to `mic_stop`/`mic_start`, restarting a healthy pipeline on a link that cannot afford it (#299 — a live violation of #140's own "a recovery mechanism must never be the thing that breaks the experience"); a control-plane drop tears down the satellite and BLE proxy with no grace, where the data plane has had `DATA_RECONNECT_GRACE_S` for exactly this reason; a device silently falling back to controller-side scoring is invisible and, on a poor link, actively harmful because the fallback is the mode that streams continuously (#302); and **#207 is now the highest-value item in the backlog** — with `owwOnDevice=on` the device streams ~32 kB/s continuously to a controller that is not allowed to act on it, which is the traffic drowning the link, and 3.0.0 turns that mode on by default.

  Also today: unified logging (#300) and device health (#301) filed, both wanted since 2026-08-20; and the log-relay design constraint that decides #300 — device logs ride the CONTROL plane, one frame per line, and log volume spikes exactly when the link is failing, so a naive relay blocks the keepalive it shares a connection with.

- 2026-08-23, end of day (**the deploy that worked and said it had not**): a clean-tree local build deployed perfectly and the dashboard announced `⚠ Device reconnected on v2.12.0-63-g99628d3 — auto-rolled back`, naming the very binary that had just booted. Two bugs, one in each half, and the pair is a good illustration of how a check can look right and match nothing that matters.

  `_extract_binary_version` knew only the old date-based scheme (`20260614-1152-dev`) while `compile.sh` moved to `git describe` for a clean tree, falling back to the date form only when dirty. So every clean build extracted nothing, got a `local-<timestamp>` label, and could never match what the device reported. **Bare `vX.Y.Z` is still deliberately not matched**, which is the half worth remembering: a Go binary embeds its dependencies' module versions, and the real firmware carries 102 occurrences across 9 distinct values — v0.41.0 (x/sys), v1.5.3 (gorilla), v1.0.3 (GoTinyAlsa), v1.1.41 (miekg/dns). Ours sorted first by luck. Only the unique `-N-gSHA` form is accepted.

  The dashboard half was worse, because it asserted something it could not know: a rollback was inferred from `firmware_ver !== targetVersion` alone, which holds only when the target is exactly what the device will report. It now takes the version the device ran BEFORE the push, so back-on-the-prior-one is the only case that earns the word.

  **And the test for that initially passed with the gate replaced by `const rolledBack = true`** — asserting that `priorVersion` appears before the wording is satisfied by the parameter name in the signature. Found by running the reintroduce-the-bug check rather than trusting green, and it now asserts the comparison itself. That is the third pattern-based check today to be the weak link (after the config-mirror window and the extractor itself), which is the argument for weighting #293 toward checks that do not encode anyone's intent.

  Also settled today: **wake arbitration survives on-device triggering** — `oww_wake` is parked in `pending_wake` and drained by the wake listener on its next frame, so capture routing, beam lock and `_wake_arbiter.claim` all still run as one sequence. That answers the question, and raises a harder one recorded on #207: the drain is driven by the continuous mic stream, so stopping that stream removes the tick the whole turn setup hangs off. #207 is therefore not "stop sending frames" but "give the setup a trigger that is not a frame", without ending up with two copies of the most delicate sequence in the controller.

- 2026-08-23, close of day (**the outcome was keyed on the phase, and the phase was never the question**): Wil, testing on hardware — a barge during THINKING recorded `cancelled` while a barge during PLAYBACK recorded `barged`. *"I mean it is a barge in, right?"* It is. Both fixes today keyed on where the turn happened to be rather than why it ended, which is how #251 came to fix the case it was not written for and how the follow-up nearly repeated it one phase over.

  Three causes, three outcomes: the dot button stops a response dead (`cancelled`), a mute stops it dead and deafens the device (`muted`), a wake word stops it in favour of another turn (`barged`). His framing settled it — the distinction people care about is whether something *replaced* the answer or just ended it. `_turn_end_reason` replaced the `_turn_barged` boolean added hours earlier, because three causes do not want three booleans, and all three early-end sites now record the reason rather than deciding from where they sit. That removes the class of bug rather than the instance.

  **The support bundle was the most valuable single artefact of the day.** It settled the evening's link mystery with data nobody had looked at: `rssi_avg` −51.4 → −69.3 dBm across three hours, `link_speed_last` 150 → **11** → 54 Mbps, `link_speed_min` reaching **1 Mbps**, and an hour spent re-associated to **2.4 GHz** — the exact failure the band and BSSID fields were added to catch. At 1 Mbps a continuous 32 kB/s stream is a quarter of the raw link rate before overhead, so the link was not lossy, it was saturated. That is the strongest evidence yet for #207, and it arrived from a device carried into a bedroom rather than from any test.

  It also produced two bugs and a config finding. **`loop_lag_peak_ms` reads 0 under the add-on and always has (#306)** — the bundle reported `0.0` beside its own log line reading `event loop stalled 881ms`, because `em_start.py` execs `em_controller.py` (so the live module is `__main__`) while both readers `import em_controller` and get a second module object holding the initial zero. It resolves correctly under docker-compose, which is how it survived: the deployment most users run is the one where it lies, and an instrument that reads "healthy" when broken is indistinguishable from a working one until the moment it is consulted. **Playback stats are nonsense across a reconnect (#307)** — `prime_wait_ms: 351813` on a 188 kB stream, next to `underruns: 0, min_depth: 24`, which is self-contradicting; one such row destroys any average over the delivery-margin fields. And **the fleet was still on `bargeInThreshold: 0.05`** two weeks after the default moved to 0.25, producing a live barge on scores of 0.072/0.111. A stored value beats a changed default — the rule was already written down, and it was met from the other direction. Both lessons are now in `controller/CLAUDE.md`.

- 2026-08-23, evening (**a guard written to prevent a regression, that could never fail**): @chr-braun's fix for the decorator sandwich in #309 arrived with a new test asserting that no helper ever steals an `@auth.require_auth` again. The regex was `r"@auth\.require_auth\n(\w+)"`, and `\w+` matches the *keyword* on the next line, never the function name — `"async"` on every real site, `"def"` on the buggy version it was written against. Reconstructing the original sandwich and running the guard over it passes. It also only looked at `require_auth`, leaving the twenty-odd `require_admin` sites uncovered. What actually protected the endpoint was the second assertion beside it, pinning `_get_system_status` to its decorator, which works but covers exactly one route.

  **That is the fourth pattern-based check in one day to be the weak link** (after the config-mirror window, `_extract_binary_version`, and the `const rolledBack = true` test that passed on its own parameter name). The pattern is consistent enough now to be worth naming: a check built from the *shape* of the bug tends to encode the author's mental model of the bug rather than the invariant that was violated. The replacement pins the invariant instead — every `@auth.require_*` is immediately followed by an `async def` whose first parameter is `request`, which is simply what a route handler *is*. Clean across all 45 decorator sites, fails on the reconstruction with `_running_controller_module async=False first_param=''`. The line-start anchor matters: unanchored it trips on two prose mentions of `@auth.require_admin` inside docstrings, one of which the fix commit itself added. More evidence for weighting #293 toward checks that do not encode anyone's intent.

  #309 and #310 merged after that (fork CI sits at `action_required` and needed approving first). #311 is held on a real hole: the wake watchdog's new link guard stands down whenever the current data connection has delivered no frames, and in the zombie-stream case it was written for — a device holding `micActive` against a *superseded* connection, Office 2026-07-16, deaf 4.7 hours — no frame ever arrives on the fresh connection, so the flag never flips and the escalation that recovers it never runs. Nothing sends `mic_start` on a new data connection, so the watchdog is the only repair. The fix is a time bound rather than a boolean. #312 (reference-counted duck) is sound; both `interrupt`/`resume_interrupted` call sites are `try`/`finally`-balanced, so the refcount cannot leak, and the follow-up that ownership is still a boolean while the duck now counts is filed as #314.

- 2026-08-23, evening (**the discovery design, and the half of it that was missing**): #106 was settled end to end with Wil, prompted by @NicFragale's #166 sitting unanswered for ten days. The discovery half is one ordered list of endpoints in `controller.json`, each carrying its own `tls_port` — static address, backup address and DNS name behave identically, so they are one list rather than three keys, and migration becomes appending an entry. Order is the migration control. `mdns: false` is a device-side setting only: turning it off on the controller does not stop a device browsing, it stops the device finding *you*, and the same zeroconf stack advertises the per-device ESPHome services that Home Assistant discovers satellites through, so a blunt controller-side switch would silently take out HA discovery of the whole fleet while voice kept working.

  **The missing half was stickiness.** A controller restart, an add-on update, a crashed binary or a poor link are not "find me a controller" events, and running the fall-through cascade on every drop means multicast browsing on the link that is already the problem — #140's own rule, with #299 as the live instance already in the tree. So the device pins the endpoint that worked and reconnects only to that, with backoff, until a grace expires; the hierarchy runs only for "never established", never for "was established and dropped". Failure is defined as no *registered* control connection within N seconds rather than TCP reachability, because a wrong or half-started controller passes a probe — and `errPending` has to count as success, or a device waiting for its approval goes hunting for a different controller.

  Two gaps fell out of that and were filed. **#315**: a control-plane drop tears down the ESPHome entities, BLE proxy and media session immediately (`em_controller.py:3275`), where the data plane has had `DATA_RECONNECT_GRACE_S` for exactly this reason; the existing guard from the Lounge incident only covers a close arriving *after* a replacement is already active, which is the rarer ordering. It was the only finding from the bedroom session never written down. **#316**: a cylon sweep for the hunting state, keeping today's orange pulse for the give-up state — two different facts, and only one of them is a problem the user should act on. It cannot be an `AnimSpec`, because that system is controller-specified and a connecting animation exists precisely when no controller can send one; it belongs with `pulseOrange` and `pulseWhite` at layer 3 of the ring ladder. Also worth knowing: the device reconnect is a flat 5s with no backoff and no jitter (`control.go:208`), so a controller restart brings the whole fleet back on the same tick, and there is no turn outcome for a link death.

- 2026-08-25 (**the EA soak read back, and three faults it had been reporting all along**): Wil granted a shell on the HA host, which turned a release decision that had been resting on an evening's listening into 15.3 hours of ea.5's own log. First lesson was mine: `ha` returned `unauthorized: missing or invalid API token` on every call and I reported Protection mode as the blocker. It was off. A plain `ssh host 'cmd'` is not a login shell, `SUPERVISOR_TOKEN` is never exported, and the failure is indistinguishable from the permission being absent — `bash -lc` fixes it. Also worth knowing: `ha apps logs` tails 100 lines with no flag to ask for more, so anything longer than a few minutes needs `curl -H "Authorization: Bearer $SUPERVISOR_TOKEN" http://supervisor/addons/<slug>/logs?lines=N`.

  **17 turns, 6 of them `ok`, and the first reading of that was wrong.** 35% looks like a broken release. Sorting by cause instead: 3 timeouts were HA-side (all `play some music on <device>`, STT fine, no TTS URL inside 30s), 1 was a false wake into 15s of room noise, 1 was Wil's own button press, and 6 were fine — including a 105-second "tell me a story" with 8.2MB of TTS, which is the case #28 and the streaming rewrite exist for. That leaves five turns actually attributable to us, and they were two bugs, not five.

  **#329 — a timed-out turn poisons the next one.** All four `pipeline_refused` turns in 15 hours immediately followed a `timeout`; none followed a good turn. The TTS wait gave up after 30s and just returned, sending no `start=False` and arming no barrier, so HA's orphaned run emitted its `RUN_END` into the *next* turn — which had not seen its own `RUN_START` — and `_ha_never_started` killed it in 3ms. This is precisely the fault #194 fixed for barge-in, on a path the barrier was never wired into, and `abort_ha_run()` already did both halves. The user-visible shape is worth recording: one slow HA intent became three dead turns, because the natural response to being ignored is to ask again, and asking again was the guaranteed failure. One of the captured utterances in that window is "Stop ignoring me." Wil's own recollection was that he asked a couple of questions, got nothing, and assumed HA — half right, and the half that was ours was invisible from the room.

  **#331 — 11.7% of every response dies at the tail.** `stream_speaker_chunks` drains whole periods, appends the limiter's look-ahead flush, then pads the remainder with `bytes(SPEAKER_BYTES - len(chunk))`. The drain leaves 0–4095 bytes and the tail adds 478, so any remainder over 3618 makes that a negative count and raises. Not a rare race — 478/4096 of all responses, and we saw 1 in 17 purely because 17 is a small number. It survived because it fires at the *end*: the audio already streamed still plays, so it reads as a clipped last syllable and a turn filed under errors, not as a failure anyone would report. The fix is not a bigger pad or a guard on the subtraction; the invariant is that a period is drained *after* the flush, and the code was never entitled to assume the tail fits.

  **#326 — the arbitration loser sits lit for 30 seconds**, found by Wil noticing it in the room rather than in the log. This is #263 meeting a second device: since #263 the Echo lights its own ring at its own wake crossing, which is what makes it feel instant and also means it lights before it can know it will lose. The cede path unwinds `oww_paused`, `last_wake`, the beam lock and the queued frames, and never touches the ring — both calls that darken it live on the turn path a ceding device never reaches. `listening_anim`'s `ttlSec: 30` was the only thing retiring it, a safety net doing a release's job.

  **#329 was a bandaid and Wil caught it before it shipped.** Asked mid-release whether we were still building with intent, I swept every `return` in `run_esphome_voice_turn` instead of answering, and found `no_speech` abandoning a live HA run exactly as `timeout` did — reached whenever a wake fires and nobody speaks, which is far more common than a slow intent. Its own comment states both halves of the contradiction three lines apart: *"there's no in-flight HA pipeline to wait on"* and *"reaching here means HA was listening and nobody spoke"*. If HA was listening, the pipeline is in flight.

  The guard therefore moved to teardown, which is a `finally` and so the one place every abandoned turn passes through, including returns nobody has written yet. `_run_finished` is set by both branches where HA genuinely ends a run, so an ordinary turn tears down with nothing to do. `end_ha_run` split out of `abort_ha_run` because teardown is not a barge and would otherwise invent `_turn_end_reason = "barged"` on every turn that merely stopped waiting; it is idempotent because a barge ends the run and then teardown runs anyway, and a second `start=False` there races the *interrupting* turn's pipeline — the precise failure the barrier exists to prevent, which would have been a poor thing to re-arm while fixing it. The lesson is the general one: a per-path fix encodes which paths you happened to look at.

  **Three tests failed on the same trap in one evening**: a source-inspection guard searching for the call it forbids, and matching instead the *comment* — or, the third time, the *docstring* — that explains why it is forbidden. All now strip comments, and the third skips past the docstring. Filed alongside the pattern already named on 2026-08-23 — the guards that break are the ones built from the shape of the bug.

- 2026-08-25 (**labels, and what `ready` was actually promising**): three PRs (#319/#320/#321) arrived in one afternoon against issues labelled `ready` within hours of filing, in a batch, without a second read. The contributor did exactly what the label invited. `ready` was carrying two meanings that had come apart: its written definition is "nothing external is blocking it", and what a contributor reads is "we have decided what to build". Those coincide most of the time and do not when a batch is filed in one sitting — nothing external blocks any of them, so the label is true as written, while whether the solution was settled had not been asked yet.

  State is now one label from `ready` / `needs-design` / `needs-decision` / `blocked` / `needs-reporter` / `needs-triage`; area says which half of the project a change lands in; the signal labels say where help is wanted. `hardware:welcome` is split out of `needs-hardware` because the two were opposites wearing one name — an invitation to somebody with a device, versus a blocker on our own bench. `claimed` exists because GitHub will not assign a non-collaborator, so an outside contributor's work in progress had no way to be visible at all, which is the same failure one step later. #140 lost its `ready`: it is an architecture, not a task, and leaving it there invites someone to implement "tolerate a lossy link" in a PR.

- 2026-08-25, late (**a compensation that outlived its cause, and the box that was never the problem**): the deliberate music-intent test asked for on ea.6 produced a different bug than the one it was testing. The turn did not time out, it was **cancelled by the device itself**: two consecutive frames at 0.206/0.238 cleared the thinking-phase low tier of 0.20, discarding the request before STT ran (`outcome=barged`, `text=''`) and reopening the mic. Wil's next utterance, which became the turn, was *"Hello. Why are you listening?"*

  **The tier existed to compensate for a fault that was later fixed elsewhere.** It was added on one observation (2026-07-12): a genuine barge plateauing at 0.240/0.242 against a threshold of 0.50, missed, answer played in full. That reading was depressed because the watcher scores a COLD model — it resets per turn and the first `FEATURE_WINDOW` chunks score reset noise as much as the room. `em_oww_warmup` was written afterwards to fix precisely that and gates this watcher until the window is clean; nobody went back and removed the compensation. What was left was a bar in the warm-noise band. Measured across 505 near-misses and 30 real detections: **no genuine wake word below 0.502, noise reaching 0.462**, and `low_tier_for`'s own docstring claiming the 0.2 floor existed "so that lowering the wake threshold cannot drag this down to somewhere noise reaches". 0.2 is where noise reaches. Removed in #337/#340, shipped as ea.7. **The general shape is worth naming: when a root cause is fixed, the mitigations added for it are now pure risk, and nothing in the process goes looking for them.**

  **Everything else that felt broken tonight was upstream, and the instrument that said so was already in the tree.** HA took **124 seconds** to route `play_media` for an intent we had given up on at 30s — the music started after Wil had left the room. Music Assistant then starved its own local stream, `Media SOURCE stall` every ~1.8s, labelled "upstream, not the device link" by `SOURCE_STALL_MS` without anyone having to think about it. That instrument is the model for #338.

  **The controller's own trace sent me the wrong way, twice.** `ha_think_ms = t_stt_ms - t_vad_end_ms` is speech recognition and nothing else — the intent lands in `tts_gen` — while its comment says "HA's own STT+intent time". So a trace reading `ha_think=+14400ms` says Home Assistant thought for 14 seconds and means Whisper transcribed for 14 seconds. Different components, different fixes. Filed with #338.
